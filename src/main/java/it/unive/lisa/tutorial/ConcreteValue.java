...
public class ConcreteValue implements BaseNonRelationalValueDomain<ConcreteValue> {
	public static final ConcreteValue BOTTOM = new ConcreteValue(Integer.MIN_VALUE),
			TOP = new ConcreteValue(Integer.MIN_VALUE);
	...
	@Override
	public ValueEnvironment<ConcreteValue> assumeBinaryExpression(ValueEnvironment<ConcreteValue> environment,
		...
		if (operator instanceof ComparisonEq) {
			if (left instanceof Variable) {
				...
				ConcreteValue v = this.eval(right, environment, dest, oracle);

				if (!v.isTop() && !v.isBottom()) {
					return environment.putState(x, new ConcreteValue(v.value));
		...
		if (operator instanceof ComparisonEq) {
			if (right instanceof Variable) {
				Variable x = (Variable) right;

				ConcreteValue v = this.eval(left, environment, dest, oracle);

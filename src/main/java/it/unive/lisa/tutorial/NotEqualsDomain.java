...
public class NotEqualsDomain extends FunctionalLattice<NotEqualsDomain, Identifier, NotEqualsDomain.SetOfIdentifiers>
        implements ValueDomain<NotEqualsDomain> {
    ...
    @Override
    public NotEqualsDomain assign(Identifier identifier, ValueExpression valueExpression, ProgramPoint programPoint,
            SemanticOracle semanticOracle) throws SemanticException {
        ...
        if (valueExpression instanceof BinaryExpression) {
            ...
            if (binaryExpression.getOperator() instanceof AdditionOperator
                    || binaryExpression.getOperator() instanceof SubtractionOperator) {
                if (binaryExpression.getLeft() instanceof Identifier
                    ...
                    if (rightConstant.getValue() instanceof Integer
                            && ((Integer) rightConstant.getValue()).intValue() != 0) {
                        result = this.putState(identifier,
                                new SetOfIdentifiers(Collections.singleton(leftVariable), false));
                    }
                }
            }
        }
    ...
    @Override
    public NotEqualsDomain smallStepSemantics(ValueExpression valueExpression, ProgramPoint programPoint,
            SemanticOracle semanticOracle) throws SemanticException {
        return this;
    }

    @Override
    public NotEqualsDomain assume(ValueExpression valueExpression, ProgramPoint programPoint,
            ProgramPoint programPoint1, SemanticOracle semanticOracle) throws SemanticException {
        NotEqualsDomain result = this;
        if (valueExpression instanceof UnaryExpression
                && ((UnaryExpression) valueExpression).getOperator() instanceof LogicalNegation) {
            ...
            if (expression instanceof BinaryExpression) {
                ...
                if (binaryExpression.getOperator() instanceof ComparisonEq
                        && binaryExpression.getLeft() instanceof Identifier
                        && binaryExpression.getLeft() instanceof Variable
                        && binaryExpression.getRight() instanceof Variable) {
                    Identifier leftVariable = (Identifier) binaryExpression.getLeft();
